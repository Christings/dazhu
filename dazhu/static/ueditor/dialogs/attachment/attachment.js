...
(function() {
	...
	UploadFile.prototype = {
		...
		getInsertList : function() {
			var i, link, data, list = [], prefix = editor
					.getOpt('fileUrlPrefix');
	...
	OnlineFile.prototype = {
		...
		pushData : function(list) {
			var i, item, img, filetype, preview, icon, _this = this, urlPrefix = editor
					.getOpt('fileManagerUrlPrefix');
				...
				if (list[i] && list[i].url) {
					...
					if ("png|jpg|jpeg|gif|bmp".indexOf(filetype) != -1) {
						...
						var ic = document.createElement('i'), textSpan = document
								.createElement('span');
						textSpan.innerHTML = list[i].url.substr(list[i].url
								.lastIndexOf('/') + 1);
                    ...
                    if (list[i].title) {
					 ...
					 $btndelete = $('<div class="file-panel">' +
	                            '<span class="cancel">' + lang.uploadDelete + '</span>' +
	                            '<span class="rotateRight">' + lang.uploadTurnRight + '</span>' +
	                            '<span class="rotateLeft">' + lang.uploadTurnLeft + '</span></div>').appendTo(item);
	     
                    $(item).on('mouseenter', function () {
                        $(this).children(".file-panel").animate({ height: 30 });
                    });
                    $(item).on('mouseleave', function () {
                        $(this).children(".file-panel").animate({ height: 0 });
                    });
 
                    $btndelete.on('click', 'span', function () {
                        var name = $(this).parents("li").attr("data-url");
                        $(this).parents("li").remove();
                        ajax.request(editor.getActionUrl(editor.getOpt('fileManagerDeleteFile')), {
                            timeout: 100000,
                            data: {
                                filename: name
                            },
